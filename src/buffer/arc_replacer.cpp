...
void ArcReplacer::RecordAccess(frame_id_t frame_id, page_id_t page_id, [[maybe_unused]] AccessType access_type) {
   ...
   *  If MRU size + MRU ghost size = replacer size: Kill the last element in the MRU ghost list, then add the page to
   * the front of MRU. Else MRU size + MRU ghost size should be smaller than replacer size (it should never be larger if
   * you do things correctly). In this case If MRU size + MRU ghost size + MFU size + MFU ghost size = 2 * replacer
   * size: Kill the last element in the MFU ghost list, then add the page to the front of MRU. Else simply add the page
   * to the front of the MRU.*/
  else {
    if (alive_lru_.size() + ghost_lru_.size() == replacer_size_) {
      ghost_lru_.kill();
    } else {
      if (alive_lru_.size() + ghost_lru_.size() + alive_lfu_.size() + ghost_lfu_.size() == 2 * replacer_size_) {
        ghost_lfu_.kill();
      }
    }
    FrameStatus frame_status = FrameStatus(page_id, frame_id, true, ArcStatus::MRU);
